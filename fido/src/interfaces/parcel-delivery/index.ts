...
export class ParcelDeliveryResolver {
  constructor(
      @Inject(CreateParcelDeliveryUseCase) private readonly createParcelDeliveryUseCase: ICreateParcelDeliveryUseCase,
      @Inject(GetParcelDeliveryUseCase) private readonly getParcelDeliveryUseCase: IGetParcelDeliveryUseCase
  ) {}

  @Query(() => [ParcelDeliveryEntity])
  getParcelDelivery(@Args('getParcelDeliveryInput') id: ParcelDeliveryEntity['id']) {
