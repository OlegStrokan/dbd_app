...
export class CreateParcelDeliveryUseCase implements ICreateParcelDeliveryUseCase {
  ...
  constructor(
      ...
      @Inject(ActionLoggerService) public readonly actionLogger: IActionLoggerService
  ) {}
  ...
  async create(dto: CreateParcelDeliveryDto): Promise<ParcelDeliveryEntity> {
    return await this.actionLogger.attemptAction({
      ...
      if (foundParcel) {
        throw new ParcelDeliveryAlreadyExistError('Parcel with current parcel number already exist ',
            {parcelDeliveryId: foundParcel.id})
      }
